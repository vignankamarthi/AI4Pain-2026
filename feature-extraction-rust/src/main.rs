//! AI4Pain 2026 feature extractor CLI.
//!
//! Forked from `Blood-Pressure-Inference-with-BVP/Feature-Extraction-Rust-Complete/src/main.rs`
//! during Phase 1 of the scaffolding plan. Adapts the CLI to AI4Pain 2026
//! conventions:
//!
//!   - Crate name reference: `ai4pain_feature_extraction`
//!   - New `--signal-type` argument tags output CSVs as `results_{split}_{signal}.csv`
//!   - New `--split` argument names the data split (train / validation / test)
//!
//! Expected invocation (per scaffolding plan Phase 1):
//!     cargo run --release -- extract \
//!         --input-dir data/raw/train/Bvp \
...
//!         --resume
//!
//! The 22 + 10 + 8 feature math is signal-agnostic and runs unchanged from
//! the BP inference reference. SignalType is purely a labeling concern: it
//! controls the output filename so downstream Python loaders can join feature
//! CSVs by (split, signal).
...
use ai4pain_feature_extraction::signal_processing;
use ai4pain_feature_extraction::types::{ExtractionCheckpoint, FeatureFramework, SignalType};
use ai4pain_feature_extraction::utils;
use ai4pain_feature_extraction::data_loader;

#[derive(Parser)]
#[command(name = "ai4pain-feature-extraction")]
#[command(about = "AI4Pain 2026 feature extractor: Catch22 + entropy + statistical features")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
#[derive(Subcommand)]
enum Commands {
    /// Extract features from a directory of CSV signal files.
    Extract {
        /// Input directory containing CSV signal files for one (split, signal) pair
        #[arg(short, long)]
        input_dir: PathBuf,

        /// Output directory for the feature CSV
        #[arg(short, long)]
        output_dir: PathBuf,

        /// Data split being processed (train / validation / test)
        #[arg(short = 's', long)]
        split: String,

        /// AI4Pain signal type (bvp / eda / resp / spo2)
        #[arg(short = 't', long)]
        signal_type: String,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Extract {
            input_dir, output_dir, split, signal_type, frameworks, workers, resume, dimensions, delays,
        } => {
            let signal = SignalType::from_str(&signal_type).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown signal type '{}'. Expected one of: bvp, eda, resp, spo2.",
                    signal_type
                )
            })?;

            let valid_splits = ["train", "validation", "test"];
            if !valid_splits.contains(&split.as_str()) {
                anyhow::bail!(
                    "Unknown split '{}'. Expected one of: train, validation, test.",
                    split
                );
            }
            ...
            log::info!("AI4Pain 2026 feature extraction starting");
            log::info!("  Split: {}", split);
            log::info!("  Signal: {}", signal.as_str());
            ...
            run_extraction(&input_dir, &output_dir, &split, signal, &fw, &dims, &taus, resume)?;
        }
...
fn run_extraction(
    ...
    split: &str,
    signal: SignalType,
    frameworks: &[FeatureFramework],
    dimensions: &[usize],
    delays: &[usize],
    resume: bool,
) -> Result<()> {
    let files = data_loader::list_csv_files(input_dir)?;
    log::info!("Found {} CSV files in {:?}", files.len(), input_dir);
    ...
    let checkpoint_name = format!("checkpoint_{}_{}.json", split, signal.as_str());
    ...
    let checkpoint_name_clone = checkpoint_name.clone();
    ...
    files.par_iter().for_each(|file| {
        ...
        if let Ok(ckpt) = checkpoint.lock() {
            if ckpt.completed_subjects.len() % 10 == 0 {
                let _ = utils::save_checkpoint(
                    &output_dir_clone.join(&checkpoint_name_clone),
                    &ckpt,
                );
            }
        }
    ...
    if !results.is_empty() {
        let out_csv = output_dir.join(format!("results_{}_{}.csv", split, signal.as_str()));
    ...
    log::info!(
        ...
        ckpt.completed_subjects.len(),
        ckpt.failed_subjects.len()
    );
    Ok(())
}
