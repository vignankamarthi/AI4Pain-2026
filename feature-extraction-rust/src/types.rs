//! Types for the AI4Pain 2026 feature extractor.
//!
//! Forked from `Blood-Pressure-Inference-with-BVP/Feature-Extraction-Rust-Complete/src/types.rs`
//! and extended with the AI4Pain-specific `SignalType` enum
//! ({Bvp, Eda, Resp, Spo2}). All other types (`FeatureFramework`,
//! `ExtractionConfig`, `FeatureRow`, `ExtractionCheckpoint`, `FailedSubject`)
//! are unchanged from the BP inference reference because they are
//! signal-agnostic.
...
// ----------------------------------------------------------------------------
// AI4Pain-specific: SignalType enum
// ----------------------------------------------------------------------------

/// Which AI4Pain 2026 signal a particular extraction run processes.
///
/// Used to tag the output CSV filenames so that downstream Python loaders can
/// join feature CSVs by (split, signal). This is the only place in the Rust
/// crate that the AI4Pain 2026 data domain leaks into the type system.
...
// ----------------------------------------------------------------------------
// FeatureFramework: catch22 / entropy / stats (verbatim from BP inference)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// ExtractionConfig (verbatim from BP inference)
// ----------------------------------------------------------------------------
...
pub struct ExtractionConfig {
    ...
    /// Sampling frequency in Hz (per-signal in AI4Pain; placeholder default)
...
impl Default for ExtractionConfig {
    fn default() -> Self {
        Self {
            ...
            delays: vec![1, 2, 3],
            // Sampling frequency placeholder; AI4Pain has per-signal rates
            // (BVP high-rate, EDA / SpO2 low-rate, RESP mid-rate). The actual
            // value is confirmed at RT-A when data arrives.
...
// ----------------------------------------------------------------------------
// FeatureRow (verbatim from BP inference, signal-agnostic)
// ----------------------------------------------------------------------------
...
pub struct FeatureRow {
    ...
    // Entropy features (10):
    // 8 ordinal-pattern + 2 amplitude-based (sample, approximate)
    #[serde(skip_serializing_if = "Option::is_none")]
...
// ----------------------------------------------------------------------------
// ExtractionCheckpoint (verbatim from BP inference)
// ----------------------------------------------------------------------------
